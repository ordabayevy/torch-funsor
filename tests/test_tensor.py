...
from funsor import FunsorTracer, ShapedTensor, Tensor, Variable
...
def test_advanced_indexing_lazy(output_shape):
    with FunsorTracer():
        x = Tensor(torch.randn((2, 3, 4) + output_shape), (Variable("i", int), Variable("j", int), Variable("k", int)))
        u = Variable("u", int, size=2)
        v = Variable("v", int, size=3)
        i = 1 - u
        j = 2 - v
        k = u + v
        ...
        expected = Tensor(expected_data, (Variable("u", int, size=2), Variable("v", int, size=3)))
...
def test_getitem_tensor():
    with FunsorTracer():
        ...
        i = Variable("i", int, size=5)
        j = Variable("j", int, size=4)
        k = Variable("k", int, size=3)
        m = Variable("m", int, size=2)
        ...
        y = Tensor(torch.randint(0, j.size, (i.size,)), (i,))
        assert_equiv(x[i, j](j=y), x[i, y])

        y = Tensor(torch.randint(0, k.size, (i.size, j.size)), (i, j))
